...
async def _send_suggestion(user: discord.User | discord.Member, text: str, guild: discord.Guild | None) -> bool:
    """Send a suggestion embed to the configured webhook. Returns True on success."""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            embed.add_field(
                name="Server",
                value=guild.name if guild else "DM",
                inline=True,
            )
...
class Suggestions(commands.Cog):
    ...
    async def prefix_suggestion(self, ctx: commands.Context, *, suggestion: str = None):
        ...
        if success:
            await ctx.reply("✅ Your suggestion has been submitted! Thanks for the feedback.", ephemeral=False)
            # Delete the original message to keep channels clean (optional — remove if unwanted)
        else:
            await ctx.reply(
                "⚠️ Suggestions are not configured yet. Please contact the bot owner.",
            )
    ...
    async def slash_suggestion(self, interaction: discord.Interaction, suggestion: str):
        if len(suggestion) > MAX_SUGGESTION_LEN:
            await interaction.response.send_message(
                f"❌ Suggestion too long. Maximum is {MAX_SUGGESTION_LEN} characters.",
                ephemeral=True,
            )
            return
        ...
        if success:
            await interaction.response.send_message(
                "✅ Your suggestion has been submitted! Thanks for the feedback.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
