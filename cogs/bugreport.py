...
async def _send_bugreport(user: discord.User | discord.Member, text: str, guild: discord.Guild | None) -> bool:
    """Send a bug report embed to the configured webhook. Returns True on success."""
    ...
    async def slash_bugreport(self, interaction: discord.Interaction, report: str):
        if len(report) > MAX_REPORT_LEN:
            await interaction.response.send_message(
                f"❌ Report too long. Maximum is {MAX_REPORT_LEN} characters.",
                ephemeral=True,
            )
            return
        ...
        if success:
            await interaction.response.send_message(
                "✅ Bug report submitted! Thanks for helping improve Jarvis.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                "⚠️ Bug reports are not configured yet. Please contact the bot owner.",
                ephemeral=True,
            )
