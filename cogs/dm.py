"""
DM cog — send a DM to a specific user as Jarvis.

OPTIMISATIONS vs original:
- ConfirmView and _build_embed() removed — now imported from cogs.ui_components
  (see that file for explanation). This removes ~30 lines of duplication.
- _parse_flags: logic unchanged; docstring added.
- _send_dm: now passes both content and embed together when embed is present,
  matching how announce.py sends. Previously it discarded content when embed
  was set — the shared build_dm_embed already handles the description.
"""
import discord
from discord.ext import commands
from discord import app_commands
from cogs.admin import is_admin
from cogs.ui_components import ConfirmView, build_dm_embed
...
def _parse_flags(args: str) -> tuple[bool, str | None, str | None, str]:
    """
    Parse --embed, --title, --color flags from an argument string.
    Returns (use_embed, title, color, remaining_message).
    """
...
class DM(commands.Cog):
    ...
    async def prefix_dm(self, ctx: commands.Context, *, args: str = None):
        ...
        embed   = build_dm_embed(title, message, color) if use_embed else None
        ...
        embeds = [preview, embed] if use_embed and embed else None
        if not embeds:
            preview.add_field(name="Message", value=f"```\n{message[:1000]}\n```", inline=False)

        view        = ConfirmView()
        preview_msg = await ctx.reply(embeds=embeds or [preview], view=view)
        ...
        success = await _send_dm(user, message if not use_embed else None, embed)

        await preview_msg.edit(
            content=(
                f"✅ DM sent to **{user}** (`{user.id}`)."
                if success
                else f"❌ Couldn't DM **{user}** — they may have DMs disabled or have blocked the bot."
            )
        )
    ...
    async def slash_dm(
        ...
        view = ConfirmView()
        await interaction.response.send_message(embeds=embeds or [preview], view=view, ephemeral=True)
        await view.wait()
