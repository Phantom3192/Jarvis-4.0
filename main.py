    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            async with bot:
                ...
                # scripts/backfill_first_interaction.py's ENTRIES (if any)
                # against the state that was just loaded. No-op once ENTRIES
                # is empty, so safe to leave wired in permanently.
